...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    DHT_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    DHT_ASSERT((window_bits >= 8 && window_bits <= 15));
    DHT_ASSERT((memlevel >= 1 && memlevel <= 9));
...
dht::ByteArray::ptr ZlibStream::getByteArray() {
